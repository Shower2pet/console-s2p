...
import {
  FlaskConical, Cpu, Droplets, Wind, DoorOpen, ShowerHead, Loader2, Timer,
...
// ── Per-station timer persistence ──────────────────────────────

const TIMER_KEY_PREFIX = "s2p_tester_timer_";
...
const loadTimer = (stationId: string): PersistedTimer | null => {
  try {
    const raw = localStorage.getItem(TIMER_KEY_PREFIX + stationId);
    if (!raw) return null;
    ...
    if (washExpired && tubExpired) { localStorage.removeItem(TIMER_KEY_PREFIX + stationId); return null; }
...
const saveTimer = (stationId: string, t: PersistedTimer) => {
  try { localStorage.setItem(TIMER_KEY_PREFIX + stationId, JSON.stringify(t)); } catch { /* */ }
};

const clearTimer = (stationId: string) => {
  try { localStorage.removeItem(TIMER_KEY_PREFIX + stationId); } catch { /* */ }
};

// Also clean up old single-key format
try { localStorage.removeItem("s2p_tester_timers"); } catch { /* */ }

// ── Component ──────────────────────────────────────────────────

const TesterStationTest = () => {
  const { stationId } = useParams<{ stationId: string }>();
  const navigate = useNavigate();
  const { user } = useAuth();

  const persisted = useMemo(() => stationId ? loadTimer(stationId) : null, [stationId]);
  ...
  useEffect(() => {
    ...
    if (washEndsAt || tubEndsAt) {
      saveTimer(stationId, { washEndsAt, washTotalSec, tubEndsAt, tubTotalSec });
    } else {
      clearTimer(stationId);
    }
  }, [washEndsAt, washTotalSec, tubEndsAt, tubTotalSec, stationId]);

  // Auto-stop at timer end
  ...
  const sendCommand = async (command: string, extras: Record<string, any> = {}) => {
    ...
    try {
      const body: Record<string, any> = { station_id: stationId, command, ...extras };
      ...
      handleAppError(err, `TesterStationTest: ${command}`);
    } finally {
      setLoadingCmd(null);
    }
  };
  ...
  const { data: station, isLoading } = useQuery({
    queryKey: ["tester-station-detail", stationId],
    queryFn: async () => {
      ...
      return data as {
        id: string; type: string; status: string | null; description: string | null;
        last_heartbeat_at: string | null; product_id: string | null;
        products?: { type: string | null; name: string | null } | null;
      };
    },
    enabled: !!stationId,
  });

  const online = station?.last_heartbeat_at
    ? Date.now() - new Date(station.last_heartbeat_at).getTime() < 100_000
    : false;
  ...
  if (!station) {
    return (
      <div className="space-y-4 animate-fade-in">
        ...
        <Button variant="outline" onClick={() => navigate("/tester/stations")} className="gap-2">
          <ArrowLeft className="h-4 w-4" /> Torna alle stazioni
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => navigate("/tester/stations")} className="shrink-0">
          <ArrowLeft className="h-5 w-5" />
        </Button>
        <div className="flex-1 min-w-0">
          <h1 className="text-2xl font-heading font-bold text-foreground flex items-center gap-2">
            <FlaskConical className="h-6 w-6 text-primary" />
            Test Hardware — {station.id}
          </h1>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card className={washIsActive ? "ring-2 ring-primary/50" : ""}>
          ...
          <CardContent className="space-y-4">
            <div className="flex gap-2">
              <Button size="sm" onClick={() => sendCommand("ON")} disabled={!!loadingCmd} className="gap-1">
                {loadingCmd === "ON" && <Loader2 className="h-3 w-3 animate-spin" />} ON
              </Button>
              <Button size="sm" variant="outline" onClick={() => sendCommand("OFF")} disabled={!!loadingCmd} className="gap-1">
            ...
            <div className="space-y-2 pt-2 border-t">
              ...
              <div className="flex items-center gap-4">
                <Slider value={[washDuration]} onValueChange={([v]) => setWashDuration(v)} min={1} max={60} step={1} className="flex-1" disabled={washIsActive} />
              ...
              <div className="flex gap-2">
                <Button size="sm" onClick={() => sendCommand("START_TIMED_WASH", { duration_seconds: washDuration * 60 })} disabled={!!loadingCmd || washIsActive} className="gap-1">
                  {loadingCmd === "START_TIMED_WASH" && <Loader2 className="h-3 w-3 animate-spin" />}
                  <ShowerHead className="h-3.5 w-3.5" /> Avvia Lavaggio
                </Button>
                <Button size="sm" variant="destructive" onClick={() => sendCommand("STOP_WASH")} disabled={!!loadingCmd} className="gap-1">
        ...
        <Card className={tubIsActive ? "ring-2 ring-primary/50" : ""}>
          ...
          <CardContent className="space-y-4">
            <div className="flex gap-2">
              <Button size="sm" onClick={() => sendCommand("ON_RELAY2")} disabled={!!loadingCmd} className="gap-1">
                {loadingCmd === "ON_RELAY2" && <Loader2 className="h-3 w-3 animate-spin" />} ON
              </Button>
              <Button size="sm" variant="outline" onClick={() => sendCommand("OFF_RELAY2")} disabled={!!loadingCmd} className="gap-1">
            ...
            <div className="space-y-2 pt-2 border-t">
              ...
              <div className="flex items-center gap-4">
                <Slider value={[tubDuration]} onValueChange={([v]) => setTubDuration(v)} min={1} max={60} step={1} className="flex-1" disabled={tubIsActive} />
              ...
              <div className="flex gap-2">
                <Button size="sm" onClick={() => sendCommand("START_TUB_CLEAN", { duration_seconds: tubDuration * 60 })} disabled={!!loadingCmd || tubIsActive} className="gap-1">
                  {loadingCmd === "START_TUB_CLEAN" && <Loader2 className="h-3 w-3 animate-spin" />} Avvia Pulizia
                </Button>
                <Button size="sm" variant="destructive" onClick={() => sendCommand("STOP_TUB_CLEAN")} disabled={!!loadingCmd} className="gap-1">
        ...
        <Card>
          ...
          <CardContent>
            <Button onClick={() => sendCommand("OPEN_GATE")} disabled={!!loadingCmd} className="gap-2">
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex gap-2">
              <Button size="sm" onClick={() => sendCommand("ON_RELAY4")} disabled={!!loadingCmd} className="gap-1">
                {loadingCmd === "ON_RELAY4" && <Loader2 className="h-3 w-3 animate-spin" />} ON
              </Button>
              <Button size="sm" variant="outline" onClick={() => sendCommand("OFF_RELAY4")} disabled={!!loadingCmd} className="gap-1">
...
export default TesterStationTest;
