...
export const useStations = (structureId?: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      let query = supabase.from("stations").select("*, structures!left(name, geo_lat, geo_lng), products!left(type, name)");
      if (structureId) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("stations")
        .select("*, structures!left(name, geo_lat, geo_lng), products!left(type, name)")
        .eq("id", id!)
        .maybeSingle();
      if (error) throw error;
      return data;
